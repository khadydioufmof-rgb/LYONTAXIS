...
<div class="mx-auto w-full max-w-7xl px-3 py-6 sm:px-6 lg:px-8">
    ...
    <section class="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        @foreach ($metrics as $metric)
            <div class="flex min-w-0 items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
                <div class="shrink-0 text-2xl font-black leading-none {{ match ($metric['tone']) { 'yellow' => 'text-[#d28c00]', 'cyan' => 'text-[#168dcc]', 'green' => 'text-emerald-600', default => 'text-[#0d47a1]' } }}">{{ $metric['value'] }}</div>
                <div class="min-w-0 text-xs font-bold leading-tight text-slate-500">{{ $metric['label'] }}</div>
            </div>
        @endforeach
    </section>
