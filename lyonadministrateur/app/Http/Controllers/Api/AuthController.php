...
class AuthController
{
    public function loginWithPassword(Request $request): JsonResponse
    {
        ...
        if (!$user || !Hash::check($credentials['password'], $user->password)) {
            return response()->json([
                'success' => false,
                'message' => 'Identifiants chauffeur invalides',
            ], 401);
        }
    ...
    public function verifyOtp(Request $request): JsonResponse
    {
        ...
        $identifier = trim($validated['phone_or_email']);
        $code = $validated['code'];
        $platform = $request->segment(3, 'client');

        if (!in_array($platform, ['admin', 'driver', 'client'], true)) {
            return response()->json([
                'success' => false,
        ...
        try {
            ...
            if ($user->role !== $platform) {
                return response()->json([
                    'success' => false,
                    'message' => 'Ce compte n’est pas autorisé sur cette plateforme',
                ], 403);
            }
            ...
            return response()->json([
                ...
                'user' => [
                    ...
                    'integral_points' => (int) $user->integral_points,
                    'coupons_count' => (int) $user->coupons_count,
                    'role' => $user->role,
                ],
            ]);
        } catch (\Exception $e) {
