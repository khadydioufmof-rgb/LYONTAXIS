...
class PageController extends Controller
{
    ...
    {
        $drivers = $this->safeDatabase(
            fn () => Driver::query()->latest()->get(),
            collect()
        );

        if ($request->filled('status')) {
            $drivers = $drivers->where('is_available', $request->status === 'available');
        }

        if ($request->filled('category')) {
            $drivers = $drivers->where('vehicle_category', $request->category);
        }

        $driverCards = $drivers->map(fn ($driver) => [
            'id' => $driver->id,
            'name' => $driver->name,
            'phone' => $driver->phone_number,
            'rating' => (float) $driver->rating,
            'trips' => (int) ($driver->total_trips ?? 0),
            'vehicle' => $driver->car_model,
            'plate' => $driver->license_plate,
            'category' => $driver->vehicle_category,
            'available' => (bool) $driver->is_available,
            'color' => $driver->car_color,
            'avatar' => strtoupper(substr((string) $driver->name, 0, 2)),
        ])->values();

        return view('pages.admin-drivers', [
            'drivers' => $driverCards,
            'categories' => $driverCards->pluck('category')->filter()->unique()->values(),
            'stats' => [
                'total' => $driverCards->count(),
                'available' => $driverCards->where('available', true)->count(),
                'rating' => round($driverCards->avg('rating') ?: 0, 1),
            ],
        ]);
    }
    ...
    public function saveDriver(Request $request)
    {
        $validated = $request->validate([
            ...
            'license_plate' => 'required|string|max:255|unique:drivers,license_plate',
            'car_color' => 'required|string|max:255',
            'vehicle_category' => 'required|string|in:Eco,Sedan,Premium,Van',
        ]);

        $this->safeDatabase(fn () => Driver::query()->create($validated), null);

        return redirect()->route('admin.drivers')->with('success', 'Chauffeur ajouté à la flotte.');
    }
